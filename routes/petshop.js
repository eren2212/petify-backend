...
/**
 * @route POST /petshop/add/profile
 * @desc Pet shop profile creation
 * @access Private
 */
  ...
  try {
    ...
    if (roleError || !roleData) {
      throw new CustomError(
        ...
        "Pet shop role not found",
        "User does not have an approved pet shop role"
      );
    }
    ...
    const petShopProfileSchema = z.object({
      shop_name: z.string().min(1, "Pet shop name is required"),
    ...
    // 3. Validation
    const validatedData = petShopProfileSchema.parse(newPetShopProfile);
    ...
    if (petShopProfileError) {
      throw new CustomError(
        ...
        "Pet shop profile creation failed",
        petShopProfileError.message
      );
    }

    const successResponse = Response.successResponse(Enum.HTTP_CODES.CREATED, {
      message: "Pet shop profile created successfully",
      petShopProfile: petShopProfileData,
    });
...
/**
 * @route GET /petshop/profile
 * @desc Pet shop profile get
 * @access Private
 */
router.get("/profile", verifyToken, async (req, res) => {
  try {
    ...
    const { data: petShopProfile, error: fetchError } = await supabase
      .from("pet_shop_profiles")
      .select("*")
    ...
    const successResponse = Response.successResponse(Enum.HTTP_CODES.OK, {
      message: "Pet shop profile retrieved successfully",
...
router.put("/profile", verifyToken, async (req, res) => {
  try {
    ...
    if (petShopProfileError) {
      throw new CustomError(
        Enum.HTTP_CODES.INT_SERVER_ERROR,
        "Pet shop profile update failed",
        petShopProfileError.message
      );
    }

    const successResponse = Response.successResponse(Enum.HTTP_CODES.CREATED, {
      message: "Pet shop profile updated successfully",
      petShopProfile: petShopProfileData,
    });
...
/**
 * @route POST /petshop/add/profile/image
 * @desc Pet shop profile image upload
 * @access Private
 */
router.post(
  "/add/profile/image",
  verifyToken,
  upload.single("petshopprofile"),
  async (req, res) => {
    try {
      if (!req.file) {
        throw new CustomError(
      ...
      if (petShopProfile.logo_url) {
        const oldFilePath = petShopProfile.logo_url;
      ...
      if (uploadError) {
        throw new CustomError(
          Enum.HTTP_CODES.INT_SERVER_ERROR,
          "Pet shop profile logo yüklenemedi",
          uploadError.message
        );
      }
      ...
      if (updateError) {
        ...
        await supabase.storage.from("petshopprofiles").remove([fileName]);

        throw new CustomError(
          Enum.HTTP_CODES.INT_SERVER_ERROR,
          "Pet shop profile logo güncellenemedi",
          updateError.message
        );
      }

      const response = Response.successResponse(Enum.HTTP_CODES.OK, {
        message: "Pet shop profile logo başarıyla güncellendi",
        logo_path: fileName,
        profile: updateData,
      });
...
router.get("/profile/logo/:filename", async (req, res) => {
  try {
    ...
    if (error || !data) {
      throw new CustomError(
        Enum.HTTP_CODES.NOT_FOUND,
        "Pet shop profile logo bulunamadı",
        "İstenen pet shop profile logo resmi bulunamadı."
      );
    }
...
router.delete("/profile/logo", verifyToken, async (req, res) => {
  try {
    ...
    if (fetchError) {
      throw new CustomError(
        Enum.HTTP_CODES.INT_SERVER_ERROR,
        "Pet shop profile logo bilgisi alınamadı",
        fetchError.message
      );
    }
    ...
    if (!petShopProfile.logo_url) {
      throw new CustomError(
        Enum.HTTP_CODES.BAD_REQUEST,
        "Silinecek pet shop profile logo bulunamadı",
        "Kullanıcının pet shop profile logo resmi bulunmuyor"
      );
    }

    const logoPath = petShopProfile.logo_url;
    ...
    if (storageError) {
      console.error(
        "Pet shop profile logo storage'dan silinemedi:",
        storageError
      );
    }
    ...
    if (updateError) {
      throw new CustomError(
        Enum.HTTP_CODES.INT_SERVER_ERROR,
        "Pet shop profile logo bilgisi silinemedi",
        updateError.message
      );
    }

    const successResponse = Response.successResponse(Enum.HTTP_CODES.OK, {
      message: "Pet shop profile logo başarıyla silindi",
